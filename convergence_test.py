...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', choices=['ImageNet'], type=str, required=True)
    ...
    parser.add_argument('--attack', choices=['GSMD','GSMD_slic','GSMD_exact'],
    ...
    parser.add_argument('--D', type=float, required=True, default=2.0)
    ...
    # ------------------------------- attacks --------------------------------
    attacks = {'GSMD': GSMD, 'GSMD_slic': GSMD_slic, 'GSMD_exact': GSMD_exact_new}
    ...
    # -------------------------------- test ----------------------------------
    resdir = f'./Convergence/{args.attack}{seq}_{jsonT}_{args.dataset}_{args.model}/'
    ...
    if args.targeted:
        ...
        results = test_untargeted_GSMD(attack(model, targeted=False, **params), dataloader)
        if results:
            if args.attack == 'GSMD' or args.attack == 'GSMD_slic' or args.attack == 'GSMD_exact':
                d = params['D']
                rho = params['rho']
                write_untargeted_results_GSMD(results[:-1], resdir + f'{args.numchunks}_{args.chunk}'+f'_{d}'+f'_{rho}')
                torch.save(results[-1],resdir + f'{args.numchunks}_{args.chunk}'+f'_{d}'+f'_{rho}_Losses.pt')
        else:
            with open(resdir + f'{args.numchunks}_{args.chunk}' + '_no_adversarial_example_found.txt', 'w') as f:
                f.write('no_adversarial_example_found')
